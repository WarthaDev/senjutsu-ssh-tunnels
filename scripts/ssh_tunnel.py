...
googleusercontent_pattern = re.compile(r"(?P<url>https?://\S+\.googleusercontent\.com)")
...
def ssh_tunnel(host: str) -> None:
    ...
    pattern = localhostrun_pattern if host == LOCALHOST_RUN else remotemoe_pattern if host == REMOTE_MOE else googleusercontent_pattern
    ...
    for _ in range(lines):
        ...
        if url_match:
            ...
            elif host == REMOTE_MOE:
                os.environ['REMOTE_MOE'] = tunnel_url
            else:
                os.environ['GOOGLEUSERCONTENT'] = tunnel_url
            break
    else:
